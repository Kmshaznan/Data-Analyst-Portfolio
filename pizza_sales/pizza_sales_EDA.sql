...
--temp table for counting bills
WITH total_orders AS(
  SELECT o.order_id, pi.pizza_id,
CAST(date as date), quantity, price, quantity*price AS bills
FROM orders AS o
JOIN order_details AS details
ON o.order_id = details.order_id
JOIN pizzas AS pi
...
SELECT TO_CHAR(date, 'Month') AS month, SUM(quantity) AS num_of_order, SUM(bills) AS revenue
FROM total_orders
GROUP BY 1
ORDER BY 1 DESC,3;
...
--finding revenue in april, may and june
SELECT pizza_id,
CASE 
    WHEN EXTRACT(MONTH FROM date) = 4 THEN 'April'
    WHEN EXTRACT(MONTH FROM date) = 5 THEN 'May'
    WHEN EXTRACT(MONTH FROM date) = 6 THEN 'June'
    END AS month, SUM(quantity) AS num_of_order, SUM(bills) AS revenue
FROM total_orders
